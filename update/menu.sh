...
UPDATE="https://raw.githubusercontent.com/savagetech56/Premier/main/update/update.sh"
GREEN='\033[0;32m'
RED='\033[0;31m'
ipsaya=$(wget -qO- ipinfo.io/ip)
data_server=$(curl -v --insecure --silent https://google.com/ 2>&1 | grep Date | sed -e 's/< Date: //')
date_list=$(date +"%Y-%m-%d" -d "$data_server")
data_ip="https://raw.githubusercontent.com/savagetech56/Premier/main/ip"
checking_sc() {
  ...
  if [[ $date_list < $useexp ]]; then
    ...
    echo -e "      \033[0;36mTelegram${NC} t.me/ToxicSavage"
    echo -e "      ${GREEN}WhatsApp${NC} wa.me/254716637803"
    echo -e "\033[1;93m────────────────────────────────────────────\033[0m"
    exit
  fi
}
...
Exp=$(curl -sS https://raw.githubusercontent.com/savagetech56/Premier/main/ip | grep $MYIP | awk '{print $3}')
fi
...
echo -e "  ${BLUE}• ${GREEN}Script Expires       ${NC}= ${YELLOW}$Exp (${NC}${RED} $dayleft Days ${NC}${YELLOW})${NC}"
